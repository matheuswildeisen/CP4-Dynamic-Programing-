...
Na Questão 1, usamos `dict` para listas de adjacência e acesso médio O(1) por vértice, `list` para pontos e vizinhos, `tuple` para arestas e resultados imutáveis e `set` para verificar rapidamente os candidatos. Na Questão 2, `list` preserva a série temporal, `dict` indexa consumo por região e por hora, `tuple` representa pares timestamp-região, `set` identifica regiões distintas e `heap` organiza os maiores picos com remoção do máximo em O(log n). Essas estruturas são construídas por `build_query_structures` e suas dimensões aparecem em `results.json`.
...
O arquivo `results.json` registra as medições da execução. Na instância atual, Greedy atende 6 pontos e deixa 1 unidade de capacidade; a DP também atende 6 pontos e obtém benefício 28.723. Na Questão 2, com 5.000 registros, a força bruta levou aproximadamente 24,57 s e D&C aproximadamente 0,16 s na máquina de execução. O arquivo `docs/contraexemplo.md` e o módulo `src/contraexemplo.py` demonstram `Greedy ≠ ótimo` na instância: capacidade 8; A=(peso 4, valor 5), B=(peso 5, valor 5), C=(peso 8, valor 9). A regra gulosa escolhe A e obtém 5, enquanto a DP escolhe C e obtém 9.
...
É necessário preencher nomes/RMs e, se aplicável, trocar a seed oficial. O layout do grafo é determinístico, exibe arestas bloqueadas em vermelho tracejado e as métricas de tempo e memória são geradas automaticamente. Os testes estão separados em `tests/test_questao1.py` e `tests/test_questao2.py`, conforme a estrutura do enunciado. Ainda dependem do grupo a criação/publicação do repositório GitHub, a URL e a preparação da explicação individual, previsão de alterações de entrada e apresentação.
